...
class OCRProcessor:
    ...
    - Auto language detection (no need to choose before upload)
    - 4 preprocessing strategies × 3 PSM configs = up to 12 attempts per image
    - Supports: images, PDF (native + scanned), DOCX
    - Auto-fixes Visual Arabic (reversed Presentation Forms) from OCR output
    """
    ...
    def _best_ocr(self, pil_image: Image.Image, language: str) -> str:
        """Try all strategies and PSMs, return the result with most text"""
    ...
    def extract_from_pil(self, pil_image: Image.Image,
        ...
        """Extract from PIL image. Returns (text, detected_language)"""
        if not language:
    ...
    def extract_from_pdf(self, pdf_bytes: bytes,
        ...
        Tries native text first; falls back to image OCR for scanned PDFs.
    ...
    def extract_from_docx(self, docx_bytes: bytes) -> Tuple[str, str]:
        """Extract text from DOCX bytes. Returns (text, language)"""
